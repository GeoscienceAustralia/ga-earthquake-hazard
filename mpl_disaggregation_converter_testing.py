...
from numpy import arange, mean, ceil, floor, log10, zeros_like, loadtxt, where
...
hist_file = 'canberra/mde_results/poe-0.02-mean-PGA-sid-0_Mag_Dist_Eps.csv'
...
####################################################################################################################
if tail.endswith('Lon_Lat_TRT.csv'):
    ...
    # set legend
    legend = ax.legend(regions, trt, title=legend, borderaxespad=1, fontsize=10)
    legend.get_title().set_fontsize('10')
    
    # add GA logo
    arr_lena = read_png('GAlogo.jpg')
    imagebox = OffsetImage(arr_lena, zoom=0.20)
    ab = AnnotationBbox(imagebox, xy=(0.14,0.05), xycoords='figure fraction', boxcoords="offset points",
					        pad=0.03, frameon=False)
    ax.add_artist(ab)
...
####################################################################################################################
    ...
    meanx = np.mean(x)
    meany = np.mean(y)
    idx = np.where((x > meanx-4.0) & (x < meanx+4.0) & (y > meany-4.0) & (y < meany+4.0))[0]
    x = np.delete(x, idx)
    y = np.delete(y, idx)
    m = np.delete(m, idx)
    p = np.delete(p, idx)
    ...
    # set cmap
    midx = np.where(mags<=7.8)[0]
    ncolours = len(mags[midx])
    ...
    #beginnig of mapping - x's and y's reversed to what would be expected
    llcrnrlon = xx[0] - bin_width1 + 5.
    llcrnrlat = yy[0] - bin_width2 + 5.
    urcrnrlon = xx[-1] + bin_width1 - 5.
    urcrnrlat = yy[-1] + bin_width2 - 5.
    lon_0 = mean([llcrnrlon, urcrnrlon])
    lat_0 = mean([llcrnrlat, urcrnrlat])
    ...
    # get plotting points for xticks    
    minylim = np.ones_like(x_axis[::4]) * llcrnrlat
    xtickx, xticky = map(x_axis[::4], minylim)    
    # relabel x ticks
    xlabels = [str(xl) for xl in x_axis[::4]]
    plt.xticks(xtickx, xlabels)
    
    # get plotting points for yticks    
    maxxlim = np.ones_like(y_axis[::4]) * urcrnrlon
    ytickx, yticky = map(maxxlim, y_axis[::4])    
    # relabel ticks
    ylabels = [str(yl) for yl in y_axis[::4]]
    plt.yticks(yticky, ylabels)
    ...
    for r in range(0,len(mags)):
        ...
        for i in range(n):
            if apoes[i] >= 0.0001 and mags[r] < 7.8:
                pl = ax.bar3d(xposm[i], yposm[i], zstart[i], dx[i], dy[i], zstop[i],
                         color=cs[r], alpha=opacity, zsort='max')
                #pl._sort_zpos = zmax - int(round(z_order[i]))
                pl._sort_zpos = int(round(z_order[i]))
                
        # get legend info
        if mags[r] < 7.8:
            exec('magl' + str(r) + ' = plt.Rectangle((0, 0), 1, 1, fc=cs[r])')
            m.append(eval('magl'+str(r)))
    ...
    # set legend
    legend = ax.legend(m, magstr, title='title', borderaxespad=1, fontsize=9)
    ...
    for k in range(0, len(zpos)):
        if zsum[k] > mode:
            ...
            modal_y = ypos[k] + (0.5 * bin_width2)
    # convert to degrees
    modal_x = np.interp(modal_x, xtickx, x_axis[::4])
    modal_y = np.interp(modal_y, yticky, y_axis[::4])
    ...
    # convert to degrees
    mean_x = np.interp(mean_x, xtickx, x_axis[::4])
    mean_y = np.interp(mean_y, yticky, y_axis[::4])
    ...
    newax = fig.add_axes(logo_bbox) #, zorder=-1)
    newax.imshow(im)
    newax.axis('off')
...
# not plotting
...
elif tail.endswith('Mag_Dist_Eps.csv'):
    print 'Mag_Dist_Eps.csv'
    ...
    epbins = []
    i = 0
    for xp, yp in zip(xpos[0:100], ypos[0:100]):
        # initiate lower bar lims
        zstart = 0
        for r in range(0,len(bins)):
            ...
            idx1 = [i, i+3, i+6] '''THINK ABOUT THIS'''
            
            # set poes
            apoes= epval[idx1]
            
            #z_order = getDistances(camera, ypos, xpos, zeros_like(apoes))
            zmax = 2*int(round(np.max(z_order)))
            
            # set upper bar lims    
            zstop = apoes
        
            if apoes[r] > 0.0001 and xpos[i] < 400:                    
                pl = ax.bar3d(xp, yp, zstart, dx[0], dy[0], zstop[r],
                         color=cs[r])#, zsort='')#, alpha=opacity, zsort='max')
                #pl._sort_zpos = zmax - int(round(z_order[i]))
                #pl._sort_zpos = z_order[i]+(n*r)
    ...
    # set legend
    legend = ax.legend(epbins, eplist, title='Epsilon', borderaxespad=1, fontsize=10, loc=2)
