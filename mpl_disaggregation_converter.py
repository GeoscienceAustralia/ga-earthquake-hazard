...
def plot_3d_hist(hist_file, xlabel, ylabel, zlabel, legend, title, site):
    ...
    # not plotting
        ...
        ncolours = len(bins)
        ncolours = 3
        cptfile = path.join('/nas/active/ops/community_safety/ehp/georisk_earthquake/hazard/DATA/cpt/BlueWhiteOrangeRed.cpt')
        cmap, zvals = cpt2colormap(cptfile, ncolours, rev=False)
        ...
        for r in range(0, len(bins)):
            ...
            zstop = apoes
            '''
            pl = ax.bar3d(xpos, ypos, zstart, dx, dy, zstop,
                             color=cs[r], alpha=opacity, zsort='max')
            pl._sort_zpos = zmax - z_order
            
            '''
            for i in range(n):
                if apoes[i] > 0:                    
                    pl = ax.bar3d(xpos[i], ypos[i], zstart[i], dx[i], dy[i], zstop[i],
                             color=cs[r])#, alpha=opacity, zsort='average')
                    pl._sort_zpos = zmax - z_order[i]
